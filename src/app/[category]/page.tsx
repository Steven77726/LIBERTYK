        ...
        <section className="page-shell py-8">
          ...
          <div className="grid gap-5 md:grid-cols-3">
            {categoryCards.map((card, index) => (
              <article
                key={card.title}
                className="relative flex min-h-80 flex-col justify-end overflow-hidden rounded-4xl p-7 text-white"
                style={{ background: card.tone }}
              >
                <span className="absolute right-6 top-6 grid size-10 place-items-center rounded-full bg-white/15 backdrop-blur">
                  {index === 0 ? <Compass size={18} /> : <MapPin size={18} />}
                </span>
                ...
                <p className="mt-2 max-w-xs text-sm leading-6 text-white/65">{card.subtitle}</p>
              </article>
            ))}
          </div>
        </section>
