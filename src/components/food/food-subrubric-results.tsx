...
export function FoodSubrubricResults() {
  ...
  useEffect(() => {
    let mounted = true;
    async function load() {
      if (!type || type === "restaurants" || type === "brunch") {
        setItems([]);
        setError("");
        return;
      }
      ...
      setError("");
      try {
        const results = await listPublishedEstablishments({ rubricSlug: "food", subrubricSlug: type });
  ...
  if (!type || type === "restaurants" || type === "brunch") return null;
