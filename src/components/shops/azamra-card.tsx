  ...
  return (
    <>
      ...
      <EntityDrawer open={open} onClose={() => setOpen(false)} title="Azamra">
        <div><div className="relative aspect-[3/4] max-h-[65vh]"><img src={assetPath(azamra.image)} alt="" className="size-full object-cover" /><div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent" /><div className="absolute bottom-6 left-6 text-white"><Shirt size={20} /><p className="mt-3 text-xs text-white/55">{azamra.type}</p><h2 className="mt-1 text-3xl font-semibold">{azamra.name}</h2></div></div><div className="space-y-5 p-6"><CustomerRating rating={azamra.rating} reviewCount={azamra.reviewCount} /><EntityActions entity={{ id: "shop-azamra", title: azamra.name, url: "/shopping/vetements/azamra", text: `${azamra.name} · ${azamra.type}` }} /><p className="text-sm leading-7 text-ink/55">{azamra.description}</p><div className="flex gap-2">{azamra.tags.map((tag) => <span key={tag} className="rounded-full bg-white px-3 py-2 text-xs">{tag}</span>)}</div></div></div>
      </EntityDrawer>
    </>
  );
}
