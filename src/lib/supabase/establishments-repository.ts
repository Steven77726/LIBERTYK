...
function rowToEstablishment(row: EstablishmentRow, photos: PhotoRow[] = [], tagMap?: Map<string, string>): EstablishmentRecord {
  ...
  return {
    ...
    deliverooUrl: ((services as Record<string, unknown>)?.deliverooUrl as string) ?? row.deliveroo_url ?? row.deliverooUrl ?? "",
    uberEatsUrl: ((services as Record<string, unknown>)?.uberEatsUrl as string) ?? row.uber_eats_url ?? row.uberEatsUrl ?? "",
    website: row.website ?? "",
