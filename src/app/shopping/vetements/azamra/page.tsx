  ...
  return (
    <section className="page-shell py-8 sm:py-12">
      ...
      <div className="overflow-hidden rounded-[2.25rem] bg-white shadow-soft">
        ...
        <div className="flex flex-col justify-between gap-7 p-7 sm:flex-row sm:p-10"><div><p className="max-w-2xl text-sm leading-7 text-ink/55">{azamra.description}</p><div className="mt-5 flex gap-2">{azamra.tags.map((tag) => <span key={tag} className="rounded-full bg-cream px-3 py-2 text-xs">{tag}</span>)}</div></div><LikeButton entity={{ id: "shop-azamra", title: azamra.name, url: "/shopping/vetements/azamra", text: `${azamra.name} · ${azamra.type}` }} className="flex h-fit items-center justify-center gap-2 rounded-xl bg-ink px-5 py-3 text-xs font-semibold text-white" /></div>
      </div>
    </section>
  );
}
